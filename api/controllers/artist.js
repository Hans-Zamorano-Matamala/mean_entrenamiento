...
function deleteArtist(req, res){
    ...
    Artist.findByIdAndDelete(artistId, (err, artistRemoved)=>{
        if(err){
            res.status(500).send({message:'Error al borrar al artista'});
        }else{
            if(!artistRemoved){
                res.status(404).send({message: "El artista " + artistId + "no existía", artistRemoved});
            }else{                
                Album.find({artist: artistRemoved._id}).remove((err, albumRemoved) => {
                    if(err){
                        res.status(500).send({message:'Error al borrar al album'});
                    }else{
                        if(!albumRemoved){
